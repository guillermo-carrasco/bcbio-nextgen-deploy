...
def load_current_resource
  ...
  unless current_installed_version.nil? || current_installed_version == ''
...
def current_installed_version
  @current_installed_version ||= begin
    ...
    p = shell_out(version_check_cmd, :user => @new_resource.user)
    p.stdout.to_s.split(delimeter)[1].to_s.strip
...
def install_package(name, version, timeout)
  ...
  shell_out!("#{expand_virtualenv(can_haz_virtualenv(@new_resource))}pip install#{expand_options(@new_resource.options)} #{name}#{v}", :timeout => timeout, :user => @new_resource.user)
end

def upgrade_package(name, version, timeout)
  v = "==#{version}" unless version.eql?('latest')
  shell_out!("#{expand_virtualenv(can_haz_virtualenv(@new_resource))}pip install --upgrade#{expand_options(@new_resource.options)} #{@new_resource.name}#{v}", :timeout => timeout, :user => @new_resource.user)
end

def remove_package(name, version, timeout)
  shell_out!("#{expand_virtualenv(can_haz_virtualenv(@new_resource))}pip uninstall -y#{expand_options(@new_resource.options)} #{@new_resource.name}", :timeout => timeout, :user => @new_resource.user)
end

def expand_virtualenv(virtualenv)
  virtualenv && "cd #{virtualenv}; source bin/activate; bin/"
end
